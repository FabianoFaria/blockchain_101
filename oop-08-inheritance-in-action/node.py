...
def load_keys():
    if wallet.load_keys():
        ...
        response = {
            'message': 'Carregar chaves falhou!!!'
        }
        return jsonify(response), 500 
...
def get_balance():
    ...
    if balance != None:
        response = {
            'message': 'Contagem do balanço concluída com sucesso!',
            'funds': balance
        }
        return jsonify(response), 200
    else:
        response = {
            'message':'Carregar balanço falhou!',
...
def broadcast_block():
    ...
    if block['index'] == blockchain.chain[-1].index + 1:
        if blockchain.add_block(block):
            response = {'message': 'Bloco adicionado.'}
            return jsonify(response), 201
        else:
...
def mine():
    ...
    if block != None:
        ...
        response = {
            'message':'Bloco foi adicionado com sucesso!',
            'block': dict_block,
            'funds': blockchain.get_balance()
        }
        return jsonify(response), 201
        ...
        response = {
            'message':'Adição de um novo bloco falhou!',
            'wallet_set_up': wallet.public_key != None 
        }
        return jsonify(response), 500
...
def resolve_conflicts():
    ...
    if replaced:
        response = {'message': 'Chain foi substituida!'}
    else:
        response = {'message':'Chain local foi mantida!'}
    return jsonify(response), 200
...
def add_node():
    ...
    if not values :
        response = {
            'message':'Nenhum dado atrelado!'
        }
        return jsonify(response), 400
    if 'node' not in values:
        response = {
            'message':'Nenhum node atrelado!'
        }
        return jsonify(response), 400
    node = values['node']
    blockchain.add_peer_node(node)
    response = {
        'message': 'Node adicionado com susseco!',
...
def remove_node(node_url):
    if node_url == '' or node_url == None:
        response = {
            'message': 'Nenhum node encontrado!'
        }
        return jsonify(response), 400
    blockchain.remove_peer_node(node_url)
    response = {
        'message': 'Node removido',
        'all_nodes': blockchain.get_peer_nodes()
    }
    return jsonify(response), 200
